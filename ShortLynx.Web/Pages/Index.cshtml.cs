...
public class IndexModel(IOptions<RedirectOptions> redirectOptions) : PageModel
{
    ...
    [
        ...
        ("No sub-country geography by default", "Country and timezone only, on every click record — never coordinates or postal code. City-level stats are available as an opt-in, aggregate-only add-on once you publish your own privacy policy — never on the individual click. A city too small to stay anonymous shows at the state or country level instead of disappearing into “Other.”"),
    ...
    [
        ...
        ("How is ShortLynx privacy-first?",
         "IP addresses are never stored raw — each is HMAC-hashed with a secret server-side key, with the current day folded in so the same visitor hashes differently each day. Do Not Track and Global Privacy Control are honored, breakdowns use k-anonymity, no sub-country geography is ever stored on an individual click (city is available only as an opt-in aggregate that falls back to state or country when a city is too small to stay anonymous, never on the click record), and exports are aggregate-only."),
