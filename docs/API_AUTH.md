...
| `GET /me/links/{id}/posts` | Publishing history with **exact per-post clicks** (each post carries its own short code, so two posts on the same platform are told apart — a referrer never could) alongside pulled engagement (likes/reposts/replies; impressions only where the platform reports them). Compare `clicks` against `likes` to separate real traffic from vanity engagement. |
