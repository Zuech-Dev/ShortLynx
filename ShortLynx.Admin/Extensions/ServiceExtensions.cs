...
public static class ServiceExtensions
{
    ...
    /// <summary>
    /// SQLite has no dedicated migrations project (see CLAUDE.md's Architecture section) and
    /// <see cref="AddShortLynxDatabase"/> above never configures a <c>MigrationsAssembly</c> for it —
    ...
    public static void EnsureSqliteSchemaCreated(IServiceProvider services, IConfiguration configuration)
    {
        ...
        using var db = services.GetRequiredService<IDbContextFactory<ShortLynxDbContext>>().CreateDbContext();
        ...
        this IServiceCollection services, IConfiguration configuration)
    {
        ...
        // Bluesky/Mastodon. The authorize/callback HTTP endpoints live in Core's SocialOAuthController,
        // which Admin can't link to directly — that controller requires Core's own JWT session, which
        // Admin's cookie auth doesn't hold (see PORTALS.md). Admin still runs these connectors for the
        // same publish/metrics paths Core does, resolved per-platform via OAuthConnectorResolver.
